...
public class HopTest {
	...
	public void accountCannotBeNull() {
		String account = null;
		Optional<Hop> hop = Hop.create(account, now, SUCCESS, 1, 2);
	...
	public void datetimeCannotBeNull() {
		...
		Optional<Hop> hop = Hop.create("account", instant, SUCCESS, 1, 2);
		assertTrue(hop.isEmpty());
	}
	...
	public void sourceWorldMustBePositiveIfProvided() {
		int source = -1;
		Optional<Hop> hop = Hop.create("account", now, SUCCESS, source, 2);
		assertTrue(hop.isEmpty());
	}
	...
	public void destinationWorldMustBePositive() {
		int destination = -2;
		Optional<Hop> hop = Hop.create("account", now, SUCCESS, 1, destination);
		assertTrue(hop.isEmpty());
	}
	...
	public void sourceAndDestinationWorldsMustBeDifferent() {
		...
		Optional<Hop> hop = Hop.create("account", now, SUCCESS, source, destination);
		assertTrue(hop.isEmpty());
	}

	@Test
	public void instantHasSecondsPrecision() {
		Hop hop = Hop.create("account", Instant.now(), SUCCESS, 301, 414).orElseThrow();
	...
	public void notEqualsNull() {
		Hop hop = Hop.create("hop", now, SUCCESS, 301, 302).orElseThrow();
	...
	public void differentAccountsNotEqual() {
		Hop first = Hop.create("first", now, SUCCESS, 301, 302).orElseThrow();
		Hop second = Hop.create("second", now, SUCCESS, 301, 302).orElseThrow();
		assertThat(first, not(equalTo(second)));
	}

	@Test
	public void differentInstantsNotEqual() {
		Hop first = Hop.create("same", now, SUCCESS, 301, 302).orElseThrow();
		Hop second = Hop.create("same", now.minusSeconds(1), SUCCESS, 301, 302).orElseThrow();
		assertThat(first, not(equalTo(second)));
	}
	...
	public void differentStatusNotEqual() {
		Hop first = Hop.create("same", now, SUCCESS, 301, 302).orElseThrow();
		Hop second = Hop.create("same", now, FAILURE, 301, 302).orElseThrow();
		assertThat(first, not(equalTo(second)));
	}

	@Test
	public void differentSourceWorldsNotEqual() {
		Hop first = Hop.create("same", now, SUCCESS, 400, 302).orElseThrow();
		Hop second = Hop.create("same", now, FAILURE, 577, 302).orElseThrow();
		assertThat(first, not(equalTo(second)));
	}

	@Test
	public void differentDestinationWorldsNotEqual() {
		Hop first = Hop.create("same", now, SUCCESS, 301, 414).orElseThrow();
		Hop second = Hop.create("same", now, FAILURE, 301, 326).orElseThrow();
		assertThat(first, not(equalTo(second)));
	}
	...
	public void sameFieldsEqual() {
		Hop first = Hop.create("same", now, SUCCESS, 301, 302).orElseThrow();
		Hop second = Hop.create("same", now, SUCCESS, 301, 302).orElseThrow();
